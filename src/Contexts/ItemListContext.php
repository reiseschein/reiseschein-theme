...
trait ItemListContext
{
    ...
    {
        return $item["data"]["prices"]["default"]["price"]["value"];
    }
    private function getRange($tmpItems){
        $tmpList    = array_map(array($this, 'mapList'), $tmpItems);
    ...
    protected function getPriceRange( $defaultSearchFactories, $options, $scope = SearchOptions::SCOPE_CATEGORY )
    {

        $currentPage    = 1;
        $itemsPerPage   = 99999;
        $itemSorting    = $options['sorting'];
        $query          = ['items' => $itemsPerPage, 'sorting' => $itemSorting];
        $searchOptions  = SearchOptions::get( $scope );
        ...
        if ( ExternalSearch::hasExternalSearch() )
        {
            /** @var ExternalSearch $externalSearch */
            $externalSearch = pluginApp( ExternalSearch::class );
            $externalSearch->page           = $currentPage;
            $externalSearch->itemsPerPage   = $itemsPerPage;
            $externalSearch->searchString   = $options['query'];
            $externalSearch->categoryId     = $options['categoryId']; 
            $externalSearch->sorting        = $itemSorting;
            ...
            if ( $externalSearch->hasResults() )
            {
                ...
                foreach( $resultVariationIds as $variationId )
                {
                    $tmpItemList[] = $searchResults[$variationId]['documents'][0];
                }
                $this->getRange($tmpItemList);
                return 0;
            }
        }
        $searchResults  = $itemSearchService->getResults( $defaultSearchFactories );
        $tmpItemList    = $searchResults['itemList']['documents'];
        $this->getRange($tmpItemList);
    }
}